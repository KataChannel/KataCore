...
async function main() {
  ...
  const messages = [
    ...
    {
      content:
        'Còn về naming convention thì sao? Mình thấy có người dùng kebab-case, có người dùng camelCase...',
      userId: newbie.id,
      timestamp: new Date('2024-01-15T09:30:00Z'),
    },
    {
      content:
        'Good question! Theo best practices:\n- **Folders**: kebab-case (`user-profile`)\n- **Components**: PascalCase (`UserProfile.tsx`)\n- **Files**: kebab-case hoặc camelCase\n- **API routes**: kebab-case',
      userId: mentor.id,
      timestamp: new Date('2024-01-15T09:35:00Z'),
    },
    {
      content:
        'Mình hay dùng barrel exports trong folder components:\n```typescript\n// components/index.ts\nexport { Header } from "./Header"\nexport { Footer } from "./Footer"\nexport { Sidebar } from "./Sidebar"\n```\nVậy import sẽ clean hơn',
      userId: developer2.id,
      timestamp: new Date('2024-01-15T09:40:00Z'),
    },
    {
      content:
        'Còn ai có tip gì về absolute imports không? Mình thấy `../../../` rất khó đọc',
      userId: newbie.id,
      timestamp: new Date('2024-01-15T09:45:00Z'),
    },
    {
      content:
        'Setup trong `tsconfig.json`:\n```json\n{\n  "compilerOptions": {\n    "baseUrl": ".",\n    "paths": {\n      "@/*": ["./src/*"],\n      "@/components/*": ["./src/components/*"],\n      "@/lib/*": ["./src/lib/*"]\n    }\n  }\n}\n```',
      userId: developer1.id,
      timestamp: new Date('2024-01-15T09:50:00Z'),
    },
    {
      content:
        'Perfect! Vậy là có thể import: `import { Button } from "@/components/ui"` thay vì `import { Button } from "../../../components/ui"`',
      userId: newbie.id,
      timestamp: new Date('2024-01-15T09:55:00Z'),
    },
    {
      content:
        'Exactly! Còn một tip nữa: dùng `_` prefix cho private folders trong app router để Next.js ignore chúng khi routing',
      userId: mentor.id,
      timestamp: new Date('2024-01-15T10:00:00Z'),
    },
    {
      content:
        'Cảm ơn mọi người! Mình đã hiểu hơn về project structure. Có tài liệu nào recommend để đọc thêm không?',
      userId: newbie.id,
