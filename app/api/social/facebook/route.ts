...
async function getPageAccessToken(pageId: string, fallbackToken?: string): Promise<string | null> {
  try {
    ...
    return fallbackToken || FACEBOOK_ACCESS_TOKEN || null;
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get credentials from headers (localStorage values) or environment variables
    const clientPageId = request.headers.get('X-Facebook-Page-Id');
    const clientAccessToken = request.headers.get('X-Facebook-Access-Token');
    
    const EFFECTIVE_PAGE_ID = clientPageId || FACEBOOK_PAGE_ID;
    const EFFECTIVE_ACCESS_TOKEN = clientAccessToken || FACEBOOK_ACCESS_TOKEN;

    // Check if credentials are configured
    const hasCredentials = EFFECTIVE_PAGE_ID && EFFECTIVE_ACCESS_TOKEN;

    console.log('Using Page ID:', EFFECTIVE_PAGE_ID);
    console.log('Has Access Token:', !!EFFECTIVE_ACCESS_TOKEN);
    ...
    if (type === 'comments') {
      ...
      try {
        const accessToken = await getPageAccessToken(pageId || '', EFFECTIVE_ACCESS_TOKEN);
        let url: string;
            ...
            if (comment.from) {
              ...
              await prisma.facebook_interactions.upsert({
                ...
                create: {
                  ...
                  facebookPageId: pageId || EFFECTIVE_PAGE_ID!,
                  type: 'COMMENT',
    ...
    if (type === 'posts') {
      try {
        const response = await fetch(
          `https://graph.facebook.com/${FACEBOOK_API_VERSION}/${EFFECTIVE_PAGE_ID}/posts?` +
            new URLSearchParams({
